...
describe('buildSvgPaths', () => {
  ...
  it('should throw if svgFolder is not provided', async () => {
    await expect(buildSvgPaths()).rejects.toEqual(new Error('rawSvgInput was not provided'))
  })

  it('should throw if glob returns an error', async () => {
    glob.mockImplementation((path, cb) => cb(new Error('Glob error'), []))
    await expect(buildSvgPaths({ globalConfig })).rejects.toEqual(new Error('Glob error'))
  })
})
